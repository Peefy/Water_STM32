...
int8 RaceModeEN = Disable;

MotorParaType MotorPara = {50,40,0};
...
void Motor_Control(void)
{
		...
		if(TimerGo.EN == false && RemoteCtr.EN == false)
		{
			...
			{		
					MoSet = (IsAlwaysRun == true) ? (StaightSpeedTemp - 10) :0;
			}
			else 
			{
		...
		#endif
		Set_Motor_PWM(LowVoltageStart(MoSet));  //慢慢给电机电压
...
int16 LowVoltageStart(int16 moSet)  //低通滤波 给定积分 使平滑加速 因为南孚电池不给力 
{
