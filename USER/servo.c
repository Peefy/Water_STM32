...
void Servo_PD(int16 GoalTemp)
{
	...
	e_bef = e;
	SerSet = SerMid + ShipDirection * SerSet_Delta;
}
...
{
	  if(ServoPara.PDcontrolerEN == true && RemoteCtr.EN == Disable && ServoPara.DebugEN == false)
		{

	     Servo_PD(ServoPara.TurnGoalAngle);
			 //Servo_PD(60 * 100); //别忘记乘以100  测试用
		}
    Set_Servo_PWM(SerSet - SerMid);
}
...
void ChangeSerMid(void)    //先调用Servo_Debug函数 就可以在线修改舵机中值并存入Flash中
{
		if(ServoPara.DebugEN == true)
		{
				up_fmq();
				SerMid = SerSet;
				WriteFlashSerMid();
...
{
		if(Readflash( FlashOffSetAddr5) == ChangeSerMidSeccessFlag)
		{		
				int16 SerMidTemp = (int16)Readflash( FlashOffSetAddr6);
				if(SerMidTemp > 1300 && SerMidTemp < 1660)  //确保读出来的数对舵机中值打角是安全的
					SerMid = SerMidTemp;
		}
}
